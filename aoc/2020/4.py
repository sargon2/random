...
for line in input.splitlines():
    ...
    for part in line.split(" "):
        ...
        if key == 'cid':
            continue
        if key == 'byr' and int(value) < 1920:
            continue
        if key == 'byr' and int(value) > 2002:
            continue
        if key == 'iyr' and int(value) < 2010:
            continue
        if key == 'iyr' and int(value) > 2020:
            continue
        if key == 'eyr' and int(value) < 2020:
            continue
        if key == 'eyr' and int(value) > 2030:
            continue
        if key == 'hgt':
            letters = value[-2:]
            try:
        ...
        if key == 'hcl':
            ...
            if len(num) != 6:
                continue
            for char in num:
                if ord(char) not in range(ord('0'), ord('9')) and char not in range(ord('a'), ord('f')):
                    continue
        if key == 'ecl':
