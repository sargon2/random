...
for($i=0;$i<=$N;$i++) {
	...
	$c = imagecolorallocate($im, $b, 0, 0);
	imagefilledrectangle($im, $i * $bw, 0, ($i + 1) * $bw - 2, 100, $c);
	imagestringup($im, $font, $i * $bw + 1, 53, $i, $black);

	$c = imagecolorallocate($im, 0, $b, 0);
	imagefilledrectangle($im, $i * $bw, 100, ($i + 1) * $bw - 2, 200, $c);
	imagestringup($im, $font, $i * $bw + 1, 153, $i, $black);

	$c = imagecolorallocate($im, 0, 0, $b);
	imagefilledrectangle($im, $i * $bw, 200, ($i + 1) * $bw - 2, 300, $c);
	imagestringup($im, $font, $i * $bw + 1, 253, $i, $black);

	$c = imagecolorallocate($im, $b, $b, $b);
	imagefilledrectangle($im, $i * $bw, 300, ($i + 1) * $bw - 2, 400, $c);
	imagestringup($im, $font, $i * $bw + 1, 353, $i, $black);
}
/*
for($i=0;$i<=260;$i++) {
	$b = 230 + ($i / 10);
